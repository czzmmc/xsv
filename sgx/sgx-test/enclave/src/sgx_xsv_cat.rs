...
fn run_cat(
    ...
    fun_name: &str,
    which: &str,
    rows1: Vec<Vec<String>>,
    rows2: Vec<Vec<String>>,
    header: bool,
    pad_flag: bool,
) -> Vec<Vec<String>> {
...
pub fn cat_rows_headers() {
    ...
    let got: Vec<Vec<String>> =
        run_cat("cat", "cat_rows_headers", "rows", rows1, rows2, true, false);
...
pub fn cat_rows_space() {
    ...
    let got: Vec<Vec<String>> = run_cat("cat", "cat_rows_space", "rows", rows1, rows2, true, false);
    assert_eq!(got, expected);
}
...
pub fn cat_cols_pad() {
    ...
    let got: Vec<Vec<String>> = run_cat("cat", "cat_cols_pad", "columns", rows1, rows2, true, true);
    assert_eq!(got, expected);
}
